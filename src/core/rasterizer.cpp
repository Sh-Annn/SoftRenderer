  ...
  for (int y = min_y; y <= max_y; ++y) {
    for (int x = min_x; x <= max_x; ++x) {
      ...
      if (alpha >= 0 && beta >= 0 && gama >= 0) {
        const float depth =
            alpha * v0.pos.z + beta * v1.pos.z + gama * v2.pos.z;
        const int idx = y * w_ + x;
        const bool pass_depth =
            !m_depth_test_enabled || (depth < depth_buf[idx]);

        if (!pass_depth) {
          continue;
        }

        const float inv_w = alpha * inv_w0 + beta * inv_w1 + gama * inv_w2;
...
void Rasterizer::draw_filled_triangle(const Vertex &v0, const Vertex &v1,
  ...
  rasterize_triangle_region(v0, v1, v2, area, shader, uniforms, min_x, min_y,
                            max_x, max_y);
}

void Rasterizer::draw_filled_triangles_tiled(
    const std::vector<ScreenTriangle> &tris, const IShader &shader,
    const ShaderUniforms &uniforms, int tile_size) {
  if (!valid() || tris.empty()) {
    return;
  }
...
#pragma omp parallel for schedule(dynamic, 1)
  for (int tile_id = 0; tile_id < tiles_x * tiles_y; tile_id++) {
    ...
    for (int tri_idx : tile_tris) {
      ...
      rasterize_triangle_region(tri.v0, tri.v1, tri.v2, tri.area, shader,
                                uniforms, min_x, min_y, max_x, max_y);
    }
  } // for tile_id
}
} // namespace core
